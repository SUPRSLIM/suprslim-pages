...
/**
 * Fetch weekmenus by status (for the control center)
 */
export async function getMenusByStatus(status) {
  try {
    const records = await airtable('Slimme Weeklijsten').select({
      filterByFormula: `{Status} = '${status}'`,
      sort: [{ field: 'Weeklijst titel', direction: 'desc' }]
    }).all();
    
    return records.map(record => ({
      id: record.id,
      title: record.get('Weeklijst titel'),
      goal: record.get('Doelgroep'),
      kcal: record.get('kcal per dag'),
      bespaartips: record.get('Bespaartips'),
      status: record.get('Status'),
    }));
  } catch (error) {
    console.error('Airtable Error [getMenusByStatus]:', error.message || error);
    return [];
  }
}
